...
export default function CTA(){
  ...
  const sub=e=>{
    ...
    window.open(`https://wa.me/917700071665?text=${encodeURIComponent(`Hi Vrushali!\n\nName: ${form.name}\nPhone: ${form.phone}\nFlat: ${form.flat||'Not specified'}\nMessage: ${form.msg||'Please get in touch.'}`)},'_blank')
    setDone(true)
  }
  ...
  const I={fontSize:9,fontWeight:800,letterSpacing:'.16em',textTransform:'uppercase',color:'rgba(242,237,223,.45)',display:'block',marginBottom:8}
  ...
  return(
    <section ref={ref} id="contact" style={{background:'var(--ink)',padding:'clamp(64px,9vw,128px) 0',position:'relative',overflow:'hidden'}} aria-label="Contact">
      ...
      <div style={{maxWidth:1320,margin:'0 auto',padding:'0 clamp(20px,5vw,64px)'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1.6fr',gap:'clamp(40px,7vw,96px)',borderTop:'1px solid rgba(255,255,255,.07)',paddingTop:'clamp(36px,5vw,64px)'}} className="cta-grid">
          ...
          <div className="rv">
            {[
              ...
              {l:'Response Time',v:'Within 2 hrs · WhatsApp',h:null},
            ].map(({l,v,h})=>(
              <div key={l} style={{marginBottom:32}}>
                ...
                {h?<a href={h} style={{fontFamily:"'Cormorant Garant',serif",fontSize:'clamp(18px,2vw,26px)',color:'var(--cream)',fontWeight:300,lineHeight:1.2}}>{v}</a>
                  :<p style={{fontFamily:"'Cormorant Garant',serif",fontSize:'clamp(18px,2vw,26px)',color:'var(--cream)',fontWeight:300,lineHeight:1.2}}>{v}</p>}
              </div>
            ))}
            <button onClick={wa} style={{fontSize:10,fontWeight:800,letterSpacing:'.16em',textTransform:'uppercase',padding:'15px 28px',minHeight:50,background:'var(--gold)',color:'var(--ink)',border:'none',borderRadius:2,display:'flex',alignItems:'center',gap:10,transition:'background .3s'}}
              onMouseEnter={e=>e.currentTarget.style.background='#dab87a'}
              onMouseLeave={e=>e.currentTarget.style.background='var(--gold)'}
            ><WAIcon/> Message on WhatsApp</button>
          </div>
            ...
            {done?(
              ...
              <form onSubmit={sub} noValidate style={{display:'flex',flexDirection:'column',gap:18}}>
                <div className="form-row" style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14}}>
                  <Fld label="Your Name *" type="text" ph="Rahul Sharma" val={form.name} set={v=>setForm(p=>({...p,name:v}))} req inp={inp} I={I}/>
                  <Fld label="WhatsApp No. *" type="tel" ph="+91 98765 43210" val={form.phone} set={v=>setForm(p=>({...p,phone:v}))} req inp={inp} I={I}/>
                </div>
                <div>
                  <label style={I}>Flat Type</label>
                  <select value={form.flat} onChange={e=>setForm(p=>({...p,flat:e.target.value}))} style={{...inp,appearance:'none',color:form.flat?'var(--cream)':'rgba(242,237,223,.35)'}}>
                    ...
                    <option value="3BHK">3 BHK</option><option value="Villa">Villa</option>
                  </select>
                </div>
                <div>
                  <label style={I}>Tell Us About Your Project</label>
                ...
                <button type="submit" disabled={!form.name||!form.phone} style={{fontSize:10,fontWeight:800,letterSpacing:'.16em',textTransform:'uppercase',padding:'16px',minHeight:52,background:form.name&&form.phone?'var(--gold)':'rgba(255,255,255,.08)',color:form.name&&form.phone?'var(--ink)':'rgba(242,237,223,.3)',border:'none',borderRadius:2,display:'flex',alignItems:'center',justifyContent:'center',gap:10,width:'100%',transition:'background .3s,color .3s'}}>
...
function Fld({label,type,ph,val,set,req,inp,I}){
  const[f,setF]=useState(false)
  return(
    <div>
      <label style={I}>{label}</label>
