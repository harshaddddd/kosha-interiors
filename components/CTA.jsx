...
export default function CTA() {
  ...
  return (
    <section
      ref={sectionRef}
      id="contact"
      style={{ background: 'var(--cream-dk)', padding: 'clamp(72px,10vw,128px) 0' }}
      aria-label="Contact and book a free consultation"
    >
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 clamp(20px,5vw,60px)' }}>
        ...
        <div style={{ marginBottom: 'clamp(48px,7vw,80px)' }}>
          ...
          >
            ...
            <em style={{ fontStyle: 'italic', color: 'var(--gold)' }}>your home?</em>
          </h2>
        </div>
        ...
        <div style={{
          ...
          <div className="reveal">
            <div style={{ display: 'flex', flexDirection: 'column', gap: 28, marginBottom: 40 }}>
              {[
                ...
                <div key={label}>
                  <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.16em', textTransform: 'uppercase', color: 'var(--gold)', marginBottom: 6 }}>{label}</p>
                  {href
          ...
          <div className="reveal reveal-d2">
            {submitted ? (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: 16, paddingTop: 8 }}>
                <div style={{ width: 48, height: 48, borderRadius: '50%', background: 'var(--gold)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="var(--ink)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
                    <polyline points="20 6 9 17 4 12"/>
                  </svg>
                </div>
                <h3 style={{ fontFamily: 'var(--ff-display)', fontSize: 28, color: 'var(--ink)', fontWeight: 400 }}>Message Sent!</h3>
              ...
              <form onSubmit={handleSubmit} noValidate style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                ...
                <div>
                  <label style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--muted)', display: 'block', marginBottom: 8 }}>Flat Type</label>
                  <select
                    value={form.flat}
                    onChange={e => setForm(p => ({...p, flat: e.target.value}))}
                    style={{
                      width: '100%', padding: '13px 16px',
                      background: 'var(--surface)', border: '1px solid rgba(13,13,13,0.12)',
                      borderRadius: 2, fontSize: 14, color: form.flat ? 'var(--ink)' : 'var(--muted)',
                ...
                <div>
                  <label style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--muted)', display: 'block', marginBottom: 8 }}>Tell Us About Your Project</label>
                  <textarea
                    rows={4}
                    value={form.message}
                    onChange={e => setForm(p => ({...p, message: e.target.value}))}
                    placeholder="E.g. 3BHK in Kharadi, modern warm design, budget around ₹12L..."
                    style={{
                      width: '100%', padding: '13px 16px',
                      background: 'var(--surface)', border: '1px solid rgba(13,13,13,0.12)',
                      borderRadius: 2, fontSize: 14, color: 'var(--ink)',
                      fontFamily: 'var(--ff-body)', resize: 'none', outline: 'none',
                    }}
                  />
                </div>
                ...
                <button
                  ...
                  style={{
                    ...
                    letterSpacing: '0.14em', textTransform: 'uppercase',
                    padding: '16px',
                    background: form.name && form.phone ? 'var(--ink)' : 'rgba(13,13,13,0.2)',
                    color: form.name && form.phone ? 'var(--cream)' : 'var(--muted)',
                    border: 'none', borderRadius: 2,
        ...
        <style>{`@media(max-width:767px){#contact > div > div:last-child{grid-template-columns:1fr!important;}}`}</style>
      </div>
    </section>
  )
}
...
function Field({ label, name, type, placeholder, value, onChange, required }) {
  ...
  return (
    <div>
      <label htmlFor={name} style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--muted)', display: 'block', marginBottom: 8 }}>{label}</label>
      <input
        ...
        style={{
          ...
          border: `1px solid ${focused ? 'var(--gold)' : 'rgba(13,13,13,0.12)'}`,
          borderRadius: 2, fontSize: 14, color: 'var(--ink)',
          fontFamily: 'var(--ff-body)', outline: 'none',
