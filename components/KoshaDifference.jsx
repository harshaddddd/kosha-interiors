...
const ITEMS=[
  {n:'01',t:'Transparent Budgeting',b:'Before a single nail is hammered, you receive an itemised breakdown covering civil, materials, carpentry, and furnishing. Every cost, laid bare. No surprises ever.',stat:'₹0 hidden'},
  {n:'02',t:'Seamless Execution',b:'Civil, POP, carpentry, painting — Vrushali coordinates every trade. One contact, zero chaos for you. You make decisions on design. We handle everything else.',stat:'1 contact'},
...
function Row({n,t,b,stat,i}){
  ...
  return(
    <div onMouseEnter={()=>setH(true)} onMouseLeave={()=>setH(false)}
      className={`rv d${i+1}`}
      style={{
        display:'grid',gridTemplateColumns:'clamp(56px,6vw,80px) 1fr clamp(80px,12vw,140px)',
        gap:'clamp(12px,3vw,40px)',alignItems:'start',
        padding:'clamp(24px,4vw,52px) 0',
        borderTop:'1px solid rgba(10,10,8,.1)',
        transition:'background .3s',
      }}
    >
      {/* Number */}
      <div>
        <span style={{fontFamily:"'Cormorant Garant',serif",fontSize:'clamp(48px,5vw,72px)',fontWeight:700,lineHeight:1,color:h?'var(--gold)':'rgba(10,10,8,.08)',transition:'color .4s var(--ease)',display:'block',letterSpacing:'-.03em'}}>{n}</span>
      </div>
      ...
      <div style={{paddingTop:8,display:'flex',justifyContent:'flex-end'}}>
        <span style={{
          ...
          fontSize:'clamp(12px,1.2vw,15px)',fontStyle:'italic',fontWeight:400,
          padding:'8px 16px',borderRadius:99,
          border:`1px solid ${h?'var(--gold)':'rgba(10,10,8,.12)'}`,
          color:h?'var(--gold)':'var(--muted)',
  ...
  return(
    <section ref={ref} id="services" style={{background:'var(--cream)',padding:'clamp(64px,9vw,128px) 0',position:'relative',overflow:'hidden'}} aria-label="The Kosha Difference">
      ...
      <div style={{maxWidth:1320,margin:'0 auto',padding:'0 clamp(20px,5vw,64px)'}}>
        ...
        {/* Header */}
        <div style={{display:'grid',gridTemplateColumns:'1fr auto',gap:20,alignItems:'flex-end',marginBottom:'clamp(40px,6vw,72px)',flexWrap:'wrap'}}>
          <div>
        ...
        {/* Rows */}
        <div>{ITEMS.map((item,i)=><Row key={item.n} {...item} i={i}/>)}</div>
        ...
        <div className="rv" style={{borderTop:'1px solid rgba(10,10,8,.1)',paddingTop:'clamp(24px,4vw,44px)',display:'flex',gap:16,flexWrap:'wrap',alignItems:'center'}}>
          <button onClick={()=>window.open(`https://wa.me/917700071665?text=${encodeURIComponent("Hi Vrushali! I have questions about interior design.")},'_blank')`}
          ...
          >Ask Vrushali</button>
          <p style={{fontSize:12,color:'var(--muted)'}}>Responds within 2 hrs · WhatsApp</p>
        </div>
      </div>
    </section>
  )
