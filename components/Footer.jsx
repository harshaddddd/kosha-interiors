  ...
  return(
    <footer style={{background:'#07070605',borderTop:'1px solid rgba(10,10,8,.08)',padding:'clamp(44px,7vw,80px) 0 clamp(20px,3vw,32px)'}} role="contentinfo">
      <div style={{maxWidth:1320,margin:'0 auto',padding:'0 clamp(20px,5vw,64px)'}}>
        <div style={{display:'grid',gridTemplateColumns:'1.4fr 1fr 1fr',gap:'clamp(24px,5vw,64px)'}} className="foot-grid">
          <div>
            <p style={{fontFamily:"'Cormorant Garant',serif",fontSize:26,fontWeight:300,color:'var(--ink)',letterSpacing:'-.01em',marginBottom:10}}>Kosha<span style={{color:'var(--gold)'}}>.</span></p>
            <p style={{fontFamily:"'Syne',sans-serif",fontSize:12,color:'var(--muted)',maxWidth:220,lineHeight:1.7,marginBottom:16}}>Pune's highest-rated woman-owned interior design studio.</p>
            <div style={{display:'flex',gap:3,marginBottom:4}}>{[...Array(5)].map((_,i)=><svg key={i} width="12" height="12" viewBox="0 0 24 24" fill="var(--gold)" aria-hidden="true"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>)}</div>
          ...
          <div>
            ...
            {['Work','Studio','Process','Reviews','Contact'].map(l=>(
              <a key={l} href={`#${l.toLowerCase()}`} style={{display:'block',fontFamily:"'Syne',sans-serif",fontSize:13,color:'var(--muted)',marginBottom:10,minHeight:28,lineHeight:'28px',transition:'color .2s'}}
          ...
          <div>
            ...
            <button onClick={wa} style={{marginTop:12,fontFamily:"'Syne',sans-serif",fontSize:9,fontWeight:800,letterSpacing:'.16em',textTransform:'uppercase',padding:'12px 22px',minHeight:44,background:'var(--gold)',color:'var(--ink)',border:'none',borderRadius:2,width:'100%'}}>Book Free Visit</button>
          </div>
        </div>
