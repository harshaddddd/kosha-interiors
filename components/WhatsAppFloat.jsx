...
export default function WhatsAppFloat(){
  ...
  useEffect(()=>{const t=setTimeout(()=>setV(true),4000);return()=>clearTimeout(t)},[])
  return(
    <div style={{position:'fixed',bottom:24,right:20,zIndex:49,opacity:v?1:0,transform:v?'scale(1)':'scale(.7) translateY(20px)',transition:'all .6s cubic-bezier(0.16,1,0.3,1)'}}>
      ...
      <style>{`@keyframes wp{0%{transform:scale(1);opacity:.7}100%{transform:scale(2.4);opacity:0}}`}</style>
    </div>
  )
}
